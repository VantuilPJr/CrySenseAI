...
import { Activity, ThermometerSun, Brain, FileText, Settings, Info, ListTree, Moon, Sun, BarChart2 } from 'lucide-react';
...
function App() {
  ...
  const { data, history, analytics, logs, config, fetchLogs, fetchConfig, fetchAnalytics } = useDashboardData();
  ...
  }, [activeTab, fetchLogs, fetchConfig, fetchAnalytics]);
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'sens': return <SensorsView data={data} history={history} />;
      case 'ia':   return <IAView data={data} />;
      case 'anal':  return <AnalyticsView analytics={analytics} uptime_s={data.uptime_s || 0} onRefresh={fetchAnalytics} />;
