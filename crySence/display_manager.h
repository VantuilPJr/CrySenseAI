// =============================================================================
// CrySense AI v2.0 — display_manager.h
// OLED SSD1306 128x64 via I2C (mesmo barramento do BME280)
// =============================================================================
#pragma once
...
void mostrarBoot() {
    ...
    _oled.clearDisplay();
    _oled.setTextSize(1);
    _oled.setCursor(20, 5);  _oled.print("CrySense AI");
    _oled.setCursor(36, 18); _oled.print("v2.0");
    _oled.setCursor(10, 35); _oled.print("Inicializando...");
    _oled.drawLine(0, 30, 127, 30, SSD1306_WHITE);
    _oled.display();
}

// --- Tela normal: temperatura + estado ---
void mostrarNormal(float temp, float umid, const char* estado, uint32_t uptime_s) {
    if (!_started) return;
    _oled.clearDisplay();
    // Header
    ...
    _oled.setTextSize(1);
    _oled.setCursor(15, 2); _oled.print("CrySense AI v2.0");
    _oled.setTextColor(SSD1306_WHITE);
...
void mostrarAlerta(const char* titulo, const char* subtitulo, float confianca, bool audioAtivo) {
    if (!_started) return;
    _oled.clearDisplay();
...
void mostrarErro(const char* msg) {
    ...
    _oled.clearDisplay();
    _oled.setTextSize(1);
    _oled.setCursor(0, 0);  _oled.print("!!! ERRO !!!");
    _oled.setCursor(0, 16); _oled.print(msg);
    _oled.display();
}
...
static void TaskHMI(void* pv) {
    ...
    while (true) {
        ...
        if (millis() - tUltimoFbSens > 60000) {
            ...
            Firebase::enviarSensores(
                ConfigManager::get().firebase_url,
        ...
        if (!gState.choroCritico) {
            const char* estadoStr = (gWebState.estado == "crise")   ? "ALERTA!" :
                                   (gWebState.estado == "calmo")     ? "Calmo"  :
                                   "Monitorando";
            mostrarNormal(
                d.valido ? d.temperatura : 0.0f,
                d.valido ? d.umidade     : 0.0f,
                estadoStr,
        ...
        if (millis() - tUltimoLogSens > 300000) {
            ...
            if (d.valido) {
                LogManager::infof("BME280: %.1f°C %.0f%% %.0fhPa",
                                  d.temperatura, d.umidade, d.pressao);
            }
        }
...
static void beginTask() {
    xTaskCreatePinnedToCore(TaskHMI, "TaskHMI", 6144, nullptr, 2, nullptr, 0);
}
