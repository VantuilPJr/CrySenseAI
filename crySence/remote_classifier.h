...
static bool _postClipHttpRaw(const CryConfig& cfg, const AudioClipMsg& clip, RemoteClassResult& rr) {
    ...
    while (sent < clip.wavLen) {
        ...
        if (n > sizeof(_txChunk)) n = sizeof(_txChunk);
        memcpy(_txChunk, clip.wavData + sent, n);
        size_t w = client.write(_txChunk, n);
        if (w != n) {
            snprintf(rr.erro, sizeof(rr.erro), "body_write_%u", (unsigned int)(sent + w));
            client.stop();
            return false;
        }
        sent += w;
        if ((sent & 0x7FFFU) == 0) vTaskDelay(pdMS_TO_TICKS(1));
    }
