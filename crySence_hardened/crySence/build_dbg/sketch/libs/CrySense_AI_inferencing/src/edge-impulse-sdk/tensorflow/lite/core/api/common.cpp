#line 1 "C:\\Arduino\\crySense_ai\\crySence_hardened\\crySence\\libs\\CrySense_AI_inferencing\\src\\edge-impulse-sdk\\tensorflow\\lite\\core\\api\\common.cpp"
...
#endif
    if (preserve_data) {
      void* old_data = tensor->data.data;
      tensor->data.data = (char*)ei_malloc(alloc_bytes);
      if (tensor->data.data && old_data) {
        memcpy(tensor->data.data, old_data, tensor->bytes >= alloc_bytes ? alloc_bytes : tensor->bytes);
        free(old_data);
      }
    } else {
